...
> **导出时间**：2026-09-23（含增量补丁：ETF 双口径 / 蓄势趋势前提。上次全量导出：2026-09-20，此前内容：v3.1 结构版：脚本移至根 scripts/ · 产物按日历目录 output/daily/<日期>/ · 含 v0.3 三池流水线步骤 9.5 · 数据通道优先 CLI · data_day 禁 today）　｜　**同步方式**：改完 prompt 后需重新导出本文件（见文末方法）
...
## 增量补丁 2026-09-23：ETF 双口径 + 蓄势趋势前提（合并状态：已写入线上，本文件待下次全量导出时并入任务一全文）

### 补丁 1：ETF 反查升级为双口径（任务一 步骤 3 替换）

```text
3. ETF反查（双口径）：
   a) 动量口径：wt ranking qt_chg_interval --asset etf --orderby ChgPct20D --min-ChgPct20D 8 --limit 40
   b) **回调再起口径（settings.etf_funnel.pullback_resume）**：收盘>MA20>MA60 且 chg60d>0 且 距60日高≤5% 且 近5日>0
      —— 抓"上升趋势中回调结束"的黄金买点（原动量口径抓不到；实证：纳指生物科技ETF 513290 20日-2.25% 却是完好多头排列）
   合并去重后：剔宽基 → 同指数留最大 → 规模≥5亿 → 前10，标注**口径来源**（动量/回调再起）+ 是否通过 trend_guard + 是否与主线重合
```

### 补丁 2：蓄势分加趋势前提（任务一 步骤 6 前置校验）

```text
6. 蓄势形态精判（≤10只）：<托管python> <蓄势引擎> --engine vcp --source westock --code <带前缀代码> --limit 120 --pretty
   **先校验趋势前提（settings.vcp.trend_precondition：chg60d>0 且 MA20>MA60）**：
   通过 = 「趋势型蓄势」正常用分；
   不通过 = 「反弹型蓄势·前提不足」→ 分数×0.6 降级，禁止据此挂突破价，只观察
   （实证：科创200ETF 589200 深跌17%反弹拿 74.9 分 > 真趋势 513290 的 62.8 分 —— 分数倒挂）
```

### 验证记录（2026-09-23 实测）

| 标的 | ETF「回调再起」口径 | 蓄势趋势前提 | 结果 |
|---|---|---|---|
| sh513290 纳指生物科技ETF | ✅ 命中 | ✅ 通过（趋势型） | **从"选不出"变为可入选** |
| sh589200 科创200ETF | ❌ 不命中 | ❌ 不通过（反弹型，74.9→44.9） | 维持排除（规模 0.33亿 亦不过闸）|
