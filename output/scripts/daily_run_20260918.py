...
MAINLINES = [
    {
        ...
        "judge": "T1✅（98% 上涨）｜T2✅（资金全市场第 1）｜T3✗（ETF 20 日 -3.66% 且 60 日 -27.83% 深跌，方向闸不通过）"
                 "｜T5 人判：AI 算力/国产替代产业逻辑在，今日为板块级放量反攻 → 判「刚起步」（资金先行、价格未点火）",
        "note": "今日最强资金方向：半导体 5 日吸金 243.78 亿居全市场第 1，且上涨广度 98%；但 ETF 仍处深跌后的反弹通道，"
                "按 T3 方向闸不算印证 → 属「资金型埋伏」而非「互证主升」。",
    },
    {
        ...
        "judge": "T1✅（元件 70% / 通信设备 86%）｜T2 分化：元件 5 日 **转净流出 -67.84 亿**（原全市场第 1）、光通信侧仍第 2"
                 "｜T3✗（通信ETF 60 日 -27.18% 深跌）→ 按「板块资金转流出＝退潮」判退潮",
    ...
    },
    {
        ...
        "limitup": "—", "pattern": "待精判",
        "judge": "T1✅（90% 上涨）｜T2✗（板块 5 日 -19.64 亿净流出）｜T3✅（ETF 20 日 +8.26% 且 60 日 +6.45%，方向闸通过）"
                 "→ 资金与价格背离，按「板块 5 日资金转流出＝退潮」判退潮",
        "note": "唯一仍满足 ETF 价格印证（T3）的主线，但板块资金连续转出、份额持续萎缩（09-17：周份额 -7.97%）——"
    ...
    },
    {
        ...
        "judge": "T1✗（种植业 45% 上涨）｜T2✗（5 日 -13.90 亿）｜ETF 20 日仅 +1.14% → 退潮延续",
        "note": "金健米业（sh600127）今日再涨停 +9.98%，但属个股情绪脉冲：20 日涨幅已达 74.63%（超高位档 60% 线），"
                "且近 10 日涨停 ≥2 根触发硬闸 → 系统判定为「情绪票」，不入趋势池（快打范畴，主线退潮期不碰）。",
    },
]

CROSS = [
    ("半导体", "✓（T1+T2）", "✗（ETF 深跌通道）", "成立但缺印证", "资金第 1 却无 ETF 搭档印证——若 ETF 后续站上 MA20 并跟涨，可升级为互证"),
    ("航运船舶", "✓（T1）", "✓（价涨，但份额萎缩）", "弱互证 / 衰竭预警", "价涨钱走，ETF 涨幅由存量资金推升"),
    ("算力硬件", "✓（T1，但核心板块资金转负）", "✗（通信ETF 60 日 -27.18%）", "双弱 / 价钱背离", "价格回暖 + PCB 资金撤出，分歧最大"),
    ("农业/粮食", "✗（涨停主线退潮）", "✗（20 日 +1.14%）", "没信号", "退潮第 7 日"),
]
...
LIMITUP = [
    ...
    ("sz001216", "华瓷股份", 3, "确认层候选（主升档）"), ("sz003026", "中晶科技", 3, "算力硬件（半导体材料）"),
...
STABLE_LIST = [  # 稳做名单（评分降序，含形态/资格）
    ...
    ("sz002080", "中材科技", 78, "玻纤/材料", "73.3 还在压", "多头池资金第 1（5日+19.21亿）；20日+11.70% 启动档上沿；盘中已触突破价", "✅稳做·观察档"),
...
def render() -> str:
    ...
    funnel_html = (
        ...
        f'→ 多头池资金强 <b>{f["多头池资金强"]}</b> → <b>稳做 {f["稳做名单"]} / 快打 {f["快打名单"]}</b>'
...
但半导体 ETF 仍处深跌通道（60 日 -27.83%），T3 不通过 → 属资金型埋伏，非互证主升。</div>
...
{card("⑤ 稳做名单（评分降序）", f'<table><tr><th>标的</th><th class="num">评分</th><th>所属主线</th><th>蓄势形态</th><th>入选理由</th><th>系统资格</th></tr>{stable_rows}</table>'
     ...
     f'<p style="margin-top:12px"><b>主升候选（确认层 25~60%）</b></p><table><tr><th>标的</th><th class="num">20日涨幅</th><th class="num">现价</th><th class="num">PE</th><th class="num">今日</th></tr>{confirm_rows}</table>')}
...
def _line_gate(line: str) -> bool:
    """方向闸是否通过（True = ETF 未处于深跌通道，T3 有效）。"""
